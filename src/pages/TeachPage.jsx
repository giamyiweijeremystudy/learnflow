// ============================================================
// pages/TeachPage.jsx — Course Management (all users)
// ============================================================
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p className="page-subtitle">
              ...
              {isAdmin && ' · Admin view (all courses visible)'}
            </p>
          </div>
      ...
      <div className="content-area">
        ...
        <div className="grid-4 animate-fade-in" style={{ marginBottom: 28 }}>
          {[
            ...
            { label: 'Total Students', value: courses.reduce((a, c) => a + (c.enrolledCount || 0), 0).toLocaleString(), icon: 'users', color: 'var(--accent2)' },
        ...
        <section style={{ marginBottom: 32 }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: 16, color: 'var(--text-secondary)' }}>YOUR COURSES</h2>
          {owned.length === 0 ? (
            <EmptyState
              icon="layers"
            ...
            <div className="grid-3">
              {owned.map((course, i) => (
                <CourseCard key={course.id} course={course} showActions onEdit={() => navigate(`/teach/edit/${course.id}`)} animationDelay={i * 60} />
              ))}
            </div>
          )}
        </section>
        ...
        {shared.length > 0 && (
          <section>
            <h2 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: 16, color: 'var(--text-secondary)' }}>SHARED WITH YOU</h2>
            <div className="grid-3">
              {shared.map((course, i) => (
                <CourseCard key={course.id} course={course} showActions onEdit={() => navigate(`/teach/edit/${course.id}`)} animationDelay={i * 60} />
              ))}
